...
<display:table name="rendezvouses" id="row" requestURI="${requestURI}"
	...
	<jstl:choose>
		<jstl:when test="${requestURI == 'rendezvous/user/list-organised.do'}">
			<display:column>
				<jstl:if
					test="${row.finalVersion == false and row.deleted == false}">
...
<jstl:choose>
	<jstl:when test="${requestURI == 'rendezvous/user/list-link.do'}">
		<input type="button" name="cancel" value="${cancelValue}"
			onclick="javascript: relativeRedir('rendezvous/user/edit.do?rendezvousId=${rendezvousSource.id}');" />
	</jstl:when>
	<jstl:otherwise>
		<input type="button" name="cancel" value="${cancelValue}"
